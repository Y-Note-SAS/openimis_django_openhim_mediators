...
class GetPatientTests(TestCase):
    ...
    def test_patch_forwards_request_body_and_returns_upstream_data(self):
        ...
        with patch(
            ...
            request = self.factory.patch(
                "/api/api_fhir_r4/Patient/123", patch_body, format="json"
            )
            response = getPatient(request, resource_id="123")
        ...
        self.assertEqual(
            args[1], "http://openimis.local:8080/api/api_fhir_r4/Patient/123/"
        )
    ...
    def test_patch_propagates_upstream_error_payload(self):
        ...
        with patch(
            ...
            request = self.factory.patch(
                "/api/api_fhir_r4/Patient/123", {"active": False}, format="json"
            )
            response = getPatient(request, resource_id="123")
