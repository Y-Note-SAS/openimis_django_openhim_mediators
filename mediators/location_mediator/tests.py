...
class FakeConfigResult:
    ...
    getLocation reads it via result.__dict__["data"], so only a "data"
...
class GetLocationTests(TestCase):
    ...
    def test_get_forwards_request_and_returns_upstream_data(self):
        ...
        self.assertEqual(
            args[1], "http://openimis.local:8080/api/api_fhir_r4/Location/"
        )
    ...
    def test_get_search_by_identifier_forwards_query_params(self):
        ...
        with patch(
            ...
            request = self.factory.get(
                "/api/api_fhir_r4/Location", {"identifier": "123"}
            )
            response = getLocation(request)
